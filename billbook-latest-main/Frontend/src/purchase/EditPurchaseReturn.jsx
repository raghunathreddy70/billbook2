...
const EditPurchaseReturn = () => {
  ...
  useEffect(() => {
    const { token } = JSON.parse(localStorage.getItem("bill_book"));
    let decodedToken;
    if (token) {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        purchaseReturnNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const [formData, setFormData] = useState({
    purchaseReturnName: "Purchases Return",
  ...
  const handlePaymentTermsChange = (terms) => {
    ...
    const moonLanding = new Date(formData.purchasesDate).getTime();
  ...
  const handleDateChange = (fieldName, date) => {
    if (date instanceof Date && !isNaN(date)) {
      ...
      if (fieldName === "paymentTerms" && formData.purchaseReturnDate) {
        const dueDate = new Date(formData.purchaseReturnDate);
      ...
      if (fieldName === "purchaseReturnDate" && formData.purchaseReturnDate) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
          dueDate: '',
      ...
      if (fieldName === "dueDate" && formData.purchaseReturnDate) {
        let days = daysBetweenDates(formData.purchaseReturnDate, date);
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
  ...
  const validateFormData = (formData) => {
    ...
    console.log(formData.bankDetails.selectBank, bankSelected, "Bank");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/purchaseReturn/update-purchasereturn/${id}`,
        { formData }
      );
  ...
  useEffect(() => {
    const fetchPurchaseReturnDetails = async (id) => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/purchaseReturn/purchases-return/${id}`
        );
        if (response.ok) {
          const purchaseReturnDetails = await response.json();
          setFormData(purchaseReturnDetails);
          // if (purchaseReturnDetails?.totalTaxPercentage !== 0) {
          //   setIsGstEnabled(true)
          // }
          if (purchaseReturnDetails?.table.length > 0) {
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.purchaseReturnNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0"> Create Purchases Return Invoice</h5>
              </div>
            </div>
            <div className="row">
                    ...
                    <div className="form-group-item border-0 mb-0">
                      <div className="row align-item-center">
                        <PanCardNumber
                          ...
                          handleSubmitPurchasereturn={handleSubmit}
                        />
                        {/* </Modal> */}
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            <label>Purchase Return Date<span className="text-danger">*</span></label>
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                className="datetimepicker form-control"
                                selected={new Date(formData.purchaseReturnDate)}
                                onChange={(date) =>
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                className="datetimepicker form-control"
                                selected={formData.dueDate && new Date(formData.dueDate)}
                                onChange={(date) =>
                                  handleDateChange("dueDate", date)
                                }
                                minDate={new Date(formData.purchaseReturnDate)}
                                dateFormat="dd-MM-yyyy"
                                showTimeInput={false}
                              ></DatePicker>
