...
const SalesReturnEdit = () => {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        salesReturnNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const handleDateChange = (fieldName, date) => {
    ...
    if (
      ...
      const timeDiff = Math.abs(
        date.getTime() - formData.salesReturnDate.getTime()
      );
      ...
      if (fieldName === "paymentTerms" && formData.salesReturnDate) {
        const dueDate = new Date(formData.salesReturnDate);
      ...
      if (fieldName === "salesReturnDate" && formData.salesReturnDate) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: date,
          dueDate: '',
      ...
      if (fieldName === "dueDate" && formData.salesReturnDate) {
        const differenceInDays = Math.floor(
          (date - formData.salesReturnDate) / (24 * 60 * 60 * 1000)
        );
        let days = daysBetweenDates(formData.salesReturnDate, date);
        setFormData((prevData) => ({
          ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${backendUrl}/api/SalesReturn/update-salesreturn/${id}`,
        { formData }
      );
      if (
  ...
  useEffect(() => {
    const fetchSallesReturnDetails = async (id) => {
      try {
        const response = await fetch(
          `${backendUrl}/api/SalesReturn/salesReturndetails/${id}`
        );
        if (response.ok) {
          const salesReturnDetails = await response.json();
          // setFormData(salesReturnDetails);
          // if (salesReturnDetails?.totalTaxPercentage !== 0) {
          //   setIsGstEnabled(true)
          // }
          if (salesReturnDetails?.table.length > 0) {
            setProductsSelected(true)
          }
          setFormData(salesReturnDetails)
          //   console.log("creditNotesDetails", purchaseDetails);
          //   console.log("creditNotesDetails", purchaseDetails.grandTotal);
        } else {
          console.error("Failed to fetch sales return details");
        }
      } catch (error) {
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.salesReturnNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Edit Sales Return</h5>
              </div>
            </div>
            <div className="row">
                  ...
                  <div className="card-body pt-4">
                    <div className="form-group-item border-0 mb-0">
                      <div className="row align-item-center">
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            <label>Sales Return Date<span className="text-danger">*</span></label>
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                className="datetimepicker form-control"
                                selected={new Date(formData.salesReturnDate)}
                                onChange={(date) =>
                                  handleDateChange("salesReturnDate", date)
                                }
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                minDate={new Date(formData.salesReturnDate)}
                                dateFormat="dd-MM-yyyy"
                                showTimeInput={false}
                              ></DatePicker>
