...
import { FaEnvelope, FaShare, FaWhatsapp } from "react-icons/fa";
...
const PaymentInView = () => {
  ...
  const name = creditNotesDetails?.name?.name;
    ...
    const fetchCreditnotesDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/paymentOutDetails/paymentoutforview/${id}`
        );
  ...
  console.log("creditNotesDetailsforpaymentView", creditNotesDetails)
      ...
      <div className={`main-wrapper ${menu ? "slide-nav" : ""}`}>
        ...
        <div className="page-wrapper">
          <div className="content container-fluid">
            ...
            <div className={`print-only-section ${menu ? "slide-nav" : ""}`}>
                              ...
                              <div className="col-md-6">
                                {creditNotesDetails && (
                                  <p className="invoice-details">
                                    ...
                                    {creditNotesDetails.paymentOutNumber}
                                  </p>
                                )}
                              </div>
          ...
          {creditNotesDetails?.paymentDate && <SendEmail endpoint={`/view-only-invoice/paymentOutDetails/paymentoutforview/${id}`} defaultMail={creditNotesDetails?.name?.email} />}
        </div>
      </div>
    </>
  );
