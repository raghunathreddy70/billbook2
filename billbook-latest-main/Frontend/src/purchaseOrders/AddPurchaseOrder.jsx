...
const AddPurchaseOrder = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:8000/api/purchaseorder/purchaseOrder/${userData?.data?._id}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        purchaseORNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/addVendor/vendors/${userData?.data?._id}`
      );
      setCustomer(
  ...
  const handleDateChange = (fieldName, date) => {
    ...
    if (
      ...
      const timeDiff = Math.abs(
        date.getTime() - formData.purchasesORDate.getTime()
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/purchaseorder/purchaseOrder",
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.purchaseORNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Create Purchase Orders</h5>
              </div>
            </div>
            <div className="row">
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                selected={formData.purchasesORDate}
                                onChange={(date) =>
                              ...
                              {formData.bankDetails.signatureImage && (
                                <>
                                  <div>
                                    <p>
                                      Uploaded file:{" "}
                                      {formData.bankDetails.signatureName}
                                    </p>
                                  </div>
                                </>
                              )}
