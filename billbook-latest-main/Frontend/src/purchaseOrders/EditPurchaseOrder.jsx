...
const EditPurchaseOrder = () => {
  ...
  // useEffect(() => {
  //   axios
  //     .get("http://localhost:8000/api/purchaseorder/purchaseOrder")
  //     .then((response) => {
  ...
  const [formData, setFormData] = useState({
    purchaseORName: "Purchases Orders",
    purchaseORNumber: 1,
    name: "",
    purchasesORDate: new Date(),
    dueDate: new Date(),
    referenceNo: "",
    products: "",
    paymentTerms: 0,
  ...
  const handlePaymentTermsChange = (terms) => {
    ...
    const moonLanding = new Date(formData.purchasesORDate).getTime();
  ...
  const handleDateChange = (fieldName, date) => {
    if (date instanceof Date && !isNaN(date)) {
      ...
      if (fieldName === "paymentTerms" && formData.purchasesORDate) {
        const dueDate = new Date(formData.purchasesORDate);
      ...
      if (fieldName === "purchasesORDate" && formData.purchasesORDate) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
          dueDate: "",
      ...
      if (fieldName === "dueDate" && formData.purchasesORDate) {
        let days = daysBetweenDates(formData.purchasesORDate, date);
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/api/purchaseorder/update-purchaseorder/${id}`,
        { formData }
      );
    ...
    const fetchPurchaseOrderDetails = async (id) => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/purchaseorder/Purchase-order/${id}`
        );
        if (response.ok) {
          const purchaseORDetails = await response.json();
          setFormData(purchaseORDetails);
          // if (purchaseORDetails?.totalTaxPercentage !== 0) {
          //   setIsGstEnabled(true);
          // }
          if (purchaseORDetails?.table.length > 0) {
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Edit Purchase Orders</h5>
              </div>
            </div>
            <div className="row">
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            ...
                            <input
                              ...
                              value={formData.purchaseORNumber}
                              readOnly
                            />
                          </div>
                        </div>
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                selected={new Date(formData.purchasesORDate)}
                                onChange={(date) =>
                                  handleDateChange("purchasesORDate", date)
                                }
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                minDate={new Date(formData.purchasesORDate)}
                                dateFormat="dd-MM-yyyy"
                                showTimeInput={false}
                              ></DatePicker>
                            </div>
