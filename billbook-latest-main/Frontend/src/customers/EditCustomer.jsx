...
const EditCustomer = ({ visible, onClose, customerId }) => {
  // const { id } = useParams();
    ...
    const fetchCustomerDetails = async (customerId) => {
      try {
        const response = await fetch(
          `${backendUrl}/api/addCustomer/getcustomerdetails/${customerId}`
        );
        if (response.ok) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${backendUrl}/api/addCustomer/update-customer/${customerId}`,
        formData
      );
  ...
  return (
    <>
      <Modal
        ...
        visible={visible}
        onCancel={onClose}
        footer={null} // You can customize the footer as need
        width={1200}
      >
