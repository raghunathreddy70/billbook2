  ...
  const handleDateChange = (fieldName, date) => {
    if (date instanceof Date && !isNaN(date)) {
      ...
      if (fieldName === "dueDate" && formData.quotationDate) {
        const days = daysBetweenDates(formData.quotationDate, date);
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
    ...
    const fetchQuotationDetails = async (id) => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/addQuotation/quotationDetails/${id}`
        );
        if (response.ok) {
          const quotationDetails = await response.json();
          setFormData(quotationDetails);
          // if (quotationDetails?.totalTaxPercentage !== 0) {
          //   setIsGstEnabled(true)
          // }
          if (quotationDetails?.table.length > 0) {
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Edit Quotation</h5>
              </div>
            </div>
            <div className="row">
                    ...
                    <div className="form-group-item border-0 mb-0">
                      <div className="row align-item-center">
                        <PanCardNumber
                          ...
                          handleSubmitQuotation={handleSubmit}
                        />
                        {/* </Modal> */}
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                minDate={new Date(formData.quotationDate)}
                                dateFormat="dd-MM-yyyy"
                                showTimeInput={false}
                              ></DatePicker>
