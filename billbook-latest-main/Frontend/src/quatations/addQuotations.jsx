...
const AddQuotations = () => {
  ...
  useEffect(() => {
    if (userData?.data?._id) {
      axios
        .get(
          `http://localhost:8000/api/addQuotation/quotations/${userData?.data?._id}`
        )
        .then((response) => {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        quotationNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.quotationNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Add Quotation</h5>
              </div>
            </div>
            <div className="row">
