...
const ViewDebitNotesInvoice = () => {
    const { id } = useParams();
    const [menu, setMenu] = useState(false);
    const [debitNotesDetails, setDebitNotesDetails] = useState([]);
    ...
    const accountName = debitNotesDetails?.bankDetails?.selectBank[0]?.accountName;
    const accountNumber =
        debitNotesDetails?.bankDetails?.selectBank[0]?.bankAccountNumber;
    const branchName = debitNotesDetails?.bankDetails?.selectBank[0]?.branchName;
    const IFSCCode = debitNotesDetails?.bankDetails?.selectBank[0]?.IFSCCode;

    useEffect(() => {
        setBalance(debitNotesDetails.payments);
    });
    ...
    const name = debitNotesDetails?.name?.name;
    const addressLine1 = debitNotesDetails?.name?.addressLine1;
    const addressLine2 = debitNotesDetails?.name?.addressLine2;
    const email = debitNotesDetails?.name?.email;
    const vendorID = debitNotesDetails?.name?.vendorId;
    const city = debitNotesDetails?.name?.billingAddress?.city;
    const country = debitNotesDetails?.name?.billingAddress?.country;
    const pincode = debitNotesDetails?.name?.billingAddress?.pincode;
    const state = debitNotesDetails?.name?.billingAddress?.state;

    useEffect(() => {
        const fetchDebitNotesDetails = async () => {
            try {
                const response = await axios.get(
                    `${backendUrl}/api/debitNotes/Debit-Notesforview/${id}`
                );
                setDebitNotesDetails(response.data);
        ...
        fetchDebitNotesDetails();
    }, [id]);



    const [customerEmail, setCustomerEmail] = useState(email);
    const debitNotesData = debitNotesDetails;
    ...
    const purchasesid = debitNotesDetails.purchasesId;
    ...
    const handleSavePayment = async () => {
        ...
        try {
            const initialResponse = await fetch(
                `${backendUrl}/api/paymentOutDetails/paymentout`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const fetchSignatureImage = async () => {
        try {
            const response = await fetch(
                debitNotesDetails?.bankDetails?.signatureImage?.url
            );
    ...
    return (
        <MainParentInvoice IFSCCode={IFSCCode} accountName={accountName} accountNumber={accountNumber} addressLine1={addressLine1} addressLine2={addressLine2} addressName={name} branchName={branchName} city={city} country={country} invoiceDetails={debitNotesDetails} invoiceDate={debitNotesDetails?.debitNotesDate} invoiceRef={invoiceContentRef} invoiceNumber={debitNotesDetails?.debitNotesNumber} menu={menu} pincode={pincode} state={state} title={"Debit Notes"} toTitle={"Debit Notes To"} />
    )
}

export default ViewDebitNotesInvoice