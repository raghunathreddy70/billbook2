...
const EditDebitNotes = () => {
  ...
  const handleDateChange = (fieldName, date) => {
    if (date instanceof Date && !isNaN(date)) {
      ...
      if (fieldName === "paymentTerms" && formData.debitNotesDate) {
        const dueDate = new Date(formData.debitNotesDate);
      ...
      if (fieldName === "debitNotesDate" && formData.debitNotesDate) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
          dueDate: "",
      ...
      if (fieldName === "dueDate" && formData.debitNotesDate) {
        let days = daysBetweenDates(formData.debitNotesDate, date);
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: isoDate,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${backendUrl}/api/debitNotes/update-debitNotes/${id}`,
        { formData }
      );
  ...
  useEffect(() => {
    const fetchDebitNotesDetails = async (id) => {
      try {
        const response = await fetch(
          `${backendUrl}/api/debitNotes/Debit-Notes/${id}`
        );
        if (response.ok) {
          const debitNotesDetails = await response.json();
          setFormData(debitNotesDetails);
          // if (debitNotesDetails?.totalTaxPercentage !== 0) {
          //   setIsGstEnabled(true)
          // }
          if (debitNotesDetails?.table.length > 0) {
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5>Purchases Return Invoice</h5>
              </div>
            </div>
            <div className="row">
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                selected={new Date(formData.debitNotesDate)}
                                onChange={(date) =>
                                  handleDateChange("debitNotesDate", date)
                                }
