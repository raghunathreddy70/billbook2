...
const AddChallans = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${backendUrl}/api/delChallen/delChallen/${userData?.data?._id}`)
      .then((response) => {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        delChanNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const handleDateChange = (fieldName, date) => {
    ...
    if (
      ...
      const timeDiff = Math.abs(
        date.getTime() - formData.delChanDate.getTime()
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${backendUrl}/api/delChallen/delchallen`,
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.delChanNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Add Delivery Challens</h5>
              </div>
            </div>
            <div className="row">
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            ...
                            <ul className="form-group-plus css-equal-heights">
                              <li>
                                <select
                                  className={`form-select ${!validation?.customerName?.isValid &&
                                    "is-invalid"
                                    }`}
                          ...
                          <div className="form-group">
                            ...
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                ...
                                minDate={formData.quotationDate}
                                dateFormat="dd-MM-yyyy"
                                showTimeInput={false}
                              ></DatePicker>
