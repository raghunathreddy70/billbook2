...
const AddCredit = () => {
  ...
  console.log("userDataasdasdfgs", userData?.data?.signatureImage);
  ...
  useEffect(() => {
    if(userData?.data?._id) {
    axios
      .get(`${backendUrl}/api/creditNotes/creditNotes/${userData?.data?._id}`)
      .then((response) => {
  ...
  useEffect(() => {
    if (datasource.length >= 0) {
      setFormData((prevData) => ({
        ...prevData,
        creditnotesNumber: getNextInvoiceNumber(),
      }));
    }
  }, [datasource]);
  ...
  const [formData, setFormData] = useState({
    invoiceName: "Credit Notes",
    businessId: userData?.data?._id,
    creditnotesNumber: 1,
    customerName: "",
    customername: "",
    creditnotesDate: new Date(),
    dueDate: new Date(),
    referenceNo: "",
    products: "",
    paymentTerms: 0,
  ...
  const handleDateChange = (fieldName, date) => {
    ...
    if (
      ...
      const timeDiff = Math.abs(
        date.getTime() - formData.creditnotesDate.getTime()
      );
      ...
      if (fieldName === "paymentTerms" && formData.creditnotesDate) {
        const dueDate = new Date(formData.creditnotesDate);
      ...
      if (fieldName === "invoiceDate" && formData.creditnotesDate) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: date,
          dueDate: "",
      ...
      if (fieldName === "dueDate" && formData.creditnotesDate) {
        const differenceInDays = Math.floor(
          (date - formData.creditnotesDate) / (24 * 60 * 60 * 1000)
        );
        let days = daysBetweenDates(formData.creditnotesDate, date);
        setFormData((prevData) => ({
          ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    // Check for payment method conditions
    if ((paymentMethod === "Bank" || paymentMethod === "Cheque") && !selectedAccount) {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/creditNotes/creditNotes",
        {
          ...formData,
          businessId: userData?.data?._id
        }
      ...
      if ((!response?.data?.iscustomer || !response?.data?.isBussinessPAN) && !response.data.invoice) {
        ...
        setPancardmodalOpen(!response?.data?.iscustomer || !response?.data?.isBussinessPAN);
        return;
      } else {
        ...
        setFormData({
          ...
          bankDetails: {
            ...
            additionalCharges: "",
            signatureName: "",
            signatureImage: null, // Ensure this is handled properly
            grandTotal: "",
            totalDiscount: 0,
          },
        });
  ...
  const fetchProductsbyId = async (selectedproduct) => {
    try {
      ...
      setSelectedProductId(response.data.products);
    } catch (error) {
  ...
  const getNextInvoiceNumber = () => {
    if (datasource.length >= 0) {
      ...
      const maxInvoiceNumber = Math.max(
        ...datasource.map((invoice) => invoice.creditnotesNumber)
      );
          ...
          <div className="content container-fluid">
            <div className="page-header">
              <div className="content-page-header justify-normal items-center space-x-2">
                <BackButton />
                <h5 className="mt-0">Creat Credit Notes</h5>
              </div>
            </div>
            <div className="row">
                  ...
                  <div className="card-body pt-4">
                    <div className="form-group-item border-0 mb-0">
                      <div className="row align-item-center">
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            <label>Credit Notes Date</label>
                            <div className="cal-icon cal-icon-info">
                              <DatePicker
                                // className=" form-control"
                                className="datetimepicker form-control "
                                selected={formData.creditnotesDate}
                                onChange={(date) =>
                                  handleDateChange("creditnotesDate", date)
                                }
