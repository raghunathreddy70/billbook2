...
const CreditPaid = ({ setDownloadData, toggleTabsState }) => {
  ...
  const columns = [
    {
      ...
      render: (text, record) => <Link to={`/credit-notes-details/${record._id}`}>{text}</Link>,
    },
    {
      title: "Credit Notes ID",
      dataIndex: "Credit",
      render: (text, record) => <Link to={`/credit-notes-details/${record.id}`}>{text}</Link>,
    ...
    },
    {
      ...
      render: (text, record) => (
        <h2 className="table-avatar">
          <Link to={`/credit-notes-details/${record._id}`} className="avatar avatar-sm me-2">
